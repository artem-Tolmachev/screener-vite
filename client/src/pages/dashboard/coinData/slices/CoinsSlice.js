...
import { DEFAULT_CHART_SETTINGS, DEFAULT_COIN_OPTION } from "../constants/defaultSettings";
import { screen, screenData } from "../constants/screenData";
const initialState = {
    btcusdt: {
...
export const coins = createSlice({
    ...
    reducers: {
        ...
        setActivePanelIcon: (state, action) => {
        ...
        coinListUpdate: (state, action) => {
            ...
            Object.keys(activedState.storeList).forEach((listKey) => {
                if (!['Красный', 'Синий', 'Зеленый', 'Розовый'].includes(listKey))
                    return;
        ...
        updateLinesEdit: (state, action) => {
            ...
            if (!isChartFromList) {
                const updatedLines = activedState.CoinData.lines.map(line => {
                    if (line.id === lineId && line.name === "HorizontalRay") {
                        const { price, timestamp } = lineObject;
            ...
            if (chartFromList) {
                ...
                activedState.storeList[activeList].item = items.map(c => c.symbol === activeSymbol ? { ...c, lines: updatedLines } : c);
            }
        }
    }
});
export const { setBtcUsdt, updateLinesEdit, removeHorzLine, addLineFlag, addCustomLine, setuFullscreen, setActivePanelIndex, setActivePanelIcon, setActivePanel, setMainScreen, newScreen, coinListUpdate, removeMarker, addMarker, listRemover, listCleaner, setActiveList, addCoinToList, delCoin, addChart, defaultLoading, changeInterval, checkedLogo, createNewList } = coins.actions;
