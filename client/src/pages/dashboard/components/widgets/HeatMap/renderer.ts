...
if (
	nextBar !== null &&
	bars[i].cells.length > 0 &&
	nextBar.cells.length > 0
) {
	const currentPrice = bars[i].cells[0].low;
	const nextPrice = nextBar.cells[0].low;
	priceStep = Math.abs(nextPrice - currentPrice);
	if (priceStep === 0) {
		priceStep = 1;
	}
}

const coordZero = priceToCoordinate(0);
const coordStep = priceToCoordinate(priceStep);

if (coordZero === null || coordStep === null) {
	// Если не можем вычислить координаты, пропускаем отрисовку
	return;
}

const stepPx = Math.abs(coordZero - coordStep);

const percentOfStep = 0.5; // 50% высоты шага

const height = stepPx * percentOfStep;

const centerPrice = (cell.low + cell.high) / 2;
const centerY = priceToCoordinate(centerPrice);

if (centerY === null) {
	// Пропускаем отрисовку ячейки, если координата не вычисляется
	return;
}

const verticalDimension = {
	position: centerY - height / 2,
	length: height,
};
